        ...
        return {
            ...
            link: function(scope, element, attrs) {
                ...
                scope.changeTimeValue = function(variable, value) {
                    ...
                    //Support for minimum hour and maximum hour
                    if (typeof scope.fullData === "undefined") {
                        //check if there is toastr notification plugin
                        if (typeof window.toastr !== "undefined"){
                            toastr.error("Please select appointment day before selecting appointment time", "Error");
                            return false;
                        }
                        //no notification plugin, return error anyway
                        return false;
                    }


                    if (variable == 'hour' && (_num < scope.fullData.hourConfig.minHour || _num > scope.fullData.hourConfig.maxHour)){
                        return false;
                    }else{
    ...
    var ADMdtpDirective = function(ADMdtp, ADMdtpConvertor, ADMdtpFactory, constants, $compile, $timeout) {
        ...
        return {
            ...
            link: function(scope, element, attrs, ctrls) {
                ...
                scope.option.hourConfig = {minHour: scope.minHour, maxHour: scope.maxHour};
                ...
                scope.updateMasterValue = function(newDate, releaseTheBeast) {
                    ...
                    scope.$applyAsync(function() {
                        ...
                        scope.fullData = {
                            hourConfig: scope.option.hourConfig,
                ...
                //Added support for minutestep attribute
                attrs.$observe("minutestep", function (_newVal) {
                    scope.$applyAsync(function() {
                        _newVal = scope.$eval(_newVal);
                        scope.option.minuteStep = _newVal;
                    });
                });

                //Added support for hours
                attrs.$observe("hours", function (_newVal) {
                    scope.$applyAsync(function() {
                        _newVal = scope.$eval(_newVal);
                        scope.time.hour = _newVal;
                    });
                });
